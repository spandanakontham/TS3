(function() {
    ...
    function setupEventListeners() {
        ...
        const addAnglesBtn = document.getElementById('add-angles-btn');
        if (addAnglesBtn) {
            addAnglesBtn.addEventListener('click', addAngle);
        }
    ...
    function addAngle() {
        ...
        angleSubbox.innerHTML = `
            <div class="angle-info-box">
                ...
                <span class="angle-stats">0 scripts, 0 variants</span>
            </div>
    ...
    function updateAngleStats(angleSubbox) {
        ...
        const scriptCount = scriptsContent.querySelectorAll(`.script-subbox[data-angle="${angleNumber}"]:not(.ai-generated)`).length;
        const variantCount = variantsContent.querySelectorAll(`.variant-subbox[data-angle="${angleNumber}"]:not(.ai-generated)`).length;
    ...
    function updateScriptStats(scriptSubbox) {
        ...
        const variantCount = variantsContent.querySelectorAll(`.variant-subbox[data-script="${scriptNumber}"]:not(.ai-generated)`).length;
    ...
    function setupTopDropdowns() {
        ...
        corporateEntityDropdown.addEventListener('change', () => {
            console.log('Corporate Entity changed:', corporateEntityDropdown.value);
        });

        regionDropdown.addEventListener('change', () => {
            console.log('Region changed:', regionDropdown.value);
        });
    }
    ...
    function updateAngleStats(angleSubbox) {
        const angleNumber = angleSubbox.dataset.angle;
        const scripts = document.querySelectorAll(`.script-subbox[data-angle="${angleNumber}"]`);
        const variants = document.querySelectorAll(`.variant-subbox[data-angle="${angleNumber}"]`);
        const statsElement = angleSubbox.querySelector('.angle-stats');
        statsElement.textContent = `${scripts.length} scripts, ${variants.length} variants`;
    }

    function updateScriptStats(scriptSubbox) {
        const scriptNumber = scriptSubbox.dataset.script;
        const variants = document.querySelectorAll(`.variant-subbox[data-script="${scriptNumber}"]`);
        const statsElement = scriptSubbox.querySelector('.script-stats');
        statsElement.textContent = `${variants.length} variants`;
    }

    function updateAllStats() {
        document.querySelectorAll('.angle-subbox').forEach(updateAngleStats);
        document.querySelectorAll('.script-subbox').forEach(updateScriptStats);
    }
